...
class ImageSubscriber(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(Image, '/i069/camera/color/image_raw', self.listener_callback, 10)
        self._publisher_location = self.create_publisher(Int32, '/preset_location' , qos_profile_location)
        self._publisher_stop = self.create_publisher(Int32, '/i069/robot_stop' , qos_profile_stop)
        self._heartrate_subscription = self.create_subscription(String, '/heart_rate', self.heartrate_callback, 10)
